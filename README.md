...
一个用于整理学堂在线作业回顾页的本地小工具。它会在你已经登录并能正常查看答案的页面中，采集每道题的题目截图和页面上可见的答案文本，并生成 Word 文档。
...
脚本会输出候选区域。把合适的 selector 填到 `src/collect.js` 顶部配置：
...
可以。只要 `xuetangx_homework_export/data.json` 和 `images/` 还在，直接运行：
